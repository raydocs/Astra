...
- `GOOGLE_CLOUD_TRANSLATE_API_KEY` (legacy/alternate name for the same Google Translate key)
- `OPENAI_API_KEY`
...
- Relay data path precedence is explicit file paths (`ASTRA_USER_DB_PATH`, `ASTRA_VIDEO_NOTE_STORE_PATH`) > `ASTRA_RELAY_DATA_DIR` > `ASTRA_DATA_DIR` > `data/server`.
- Existing user DB files are migrated in-place with generated `id`, `billingEmail`, and `createdAt` fields when missing.
- Current local plan policy is temporarily free-first:
  - `free`: `google_translate` + `openai` + `gemini`, `2000` daily requests, `500_000` daily characters, `120` requests/minute
  - `pro`: same default provider access, `2000` daily requests, `500_000` daily characters, `120` requests/minute
- Google Translate uses Cloud Translation Basic v2 (`POST https://translation.googleapis.com/language/translate/v2`) with the `nmt` model and does not route through OpenRouter.
