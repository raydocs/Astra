...
Use `evaluateAstraProductionMetricsExportPacket()` before setting `productionMetricsExportAttached` true. Local V0 diagnostics, metrics dictionaries, the aggregate mobile retention ops summary, and instrumentation tests do not satisfy this packet without production/cohort export rows.
