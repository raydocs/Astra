...
- production_metrics_export: Attach production/cohort metric dashboard exports with valid non-reversed shared YYYY-MM-DD..YYYY-MM-DD date range, ISO exported-at timestamp, export id, stable non-weak digest/checksum, query version, category-aligned non-duplicated metric ids, URL or repo artifact-path evidence/privacy links, and owner/date containing a real calendar YYYY-MM-DD before metric maturity claims.
