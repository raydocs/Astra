...
- run id, job name, uploaded artifact id, artifact digest/checksum, and artifact manifest path (`data/bench-results/quality-gate-manifest.json` or URL/repo artifact path);
- downloadable non-local artifact URL;
- target commit/SHA as a 7–40 character hex git SHA;
...
- run id, job name, uploaded artifact id, artifact digest/checksum, and artifact manifest path (`data/bench-live-results/live-bench-manifest.json` or URL/repo artifact path);
- downloadable non-local artifact URL;
- target commit/SHA as a 7–40 character hex git SHA;
- owner/date containing a real calendar `YYYY-MM-DD`;
...
CI artifact upload wiring and a CI artifact packet intake guard exist in repo. Final completion still requires actual CI quality/live-browser run URLs, run/job/artifact identity, distinct uploaded artifact ids/URLs, artifact digests/checksums, artifact manifests, and downloadable uploaded artifacts for the target commit/worktree.
