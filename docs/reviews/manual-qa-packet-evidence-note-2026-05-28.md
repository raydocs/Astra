...
| Manual QA packet intake | `evaluateAstraMacroManualQaEvidencePacket()` | Requires every Section 6/7/13/14/24/32 row to be present, run, evidence-backed, and verdicted `pass` or `pass-with-downgrade`. | This validates supplied evidence; it does not perform manual/browser QA. |
...
Every row needs owner/date, environment, evidence link, and verdict `pass` or `pass-with-downgrade`. A row with `not-run`, `fail`, blank owner/date, blank environment, or blank evidence link keeps `manualQaChecklistComplete` false.
