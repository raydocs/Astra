...
| 隐私模式 (Beta → core policy) | **Partial** — privacy-mode context sanitization already exists, and live + holdout privacy page-translation gates are now green, but it is not yet a full system-wide benchmark gate or proof-covered capability | `bench=partial`, `live=green`, `holdout=green`, `proof=partial` | Context-aware AI reading UX implies strong privacy expectations | Product positioning strongly implies privacy guarantees | `protocol`, `coverage`, `runtime` | **D** | Deterministic privacy assertions green across request surfaces; live privacy-mode toggles green; should-not-leak holdouts green; benchmark pack can fail on privacy regressions |
