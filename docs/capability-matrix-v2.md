...
| 鼠标悬停翻译 | **Partial → strong base** — deterministic hover support is already implemented and covered, but live + holdout parity is still missing | `bench=green`, `live=missing`, `holdout=missing`, `proof=partial` | Inline / selection-adjacent UX is a strong inspiration source | Explicit hover-translation surface | `coverage`, `ux` | **C** | Live hover lane green; moving-target and overlay-interference holdouts green; no conflicts with selection/input/float-ball flows |
| 字幕文件翻译 | **Gap** — current subtitle support targets on-page caption/subtitle tracks rather than local subtitle-file ingest/export | `bench=missing`, `live=missing`, `holdout=missing`, `proof=missing` | Adjacent to subtitle concepts, but file ingest/export is a separate workflow | Public document/subtitle-file translation surface | `new-subsystem`, `runtime`, `protocol` | **C** | `.srt/.vtt` parser/serializer deterministic lanes green; upload/preview live flow green; malformed timing and overlapping-cue holdouts green; parse/translate/export failures are operator-visible |
| 输入框翻译 | **Strong base** — deterministic bench + live input translation already exist and are stable | `bench=green`, `live=green`, `holdout=partial`, `proof=green` | Inline authoring assist and selection-adjacent UX offer strong interaction inspiration | Explicit input-box translation surface | `coverage`, `ux` | **B** | Textarea/contenteditable/cursor-preservation deterministic lanes green; multi-field live flow green; delayed hydration and repeated-edit holdouts green; reporting distinguishes trigger/translate/writeback failures |
