...
Use `evaluateAstraProductionMetricsExportPacket()` before setting `productionMetricsExportAttached` true in the final macro evidence artifact. Each production/cohort export row must include category, date range, cohort definition, dashboard/query source, evidence link, owner/date, and privacy-review link.
