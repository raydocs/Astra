...
describe("applyReview", () => {
  it("promotes on correct", () => {
    const fields = createDefaultSrsFields(T0)
    const result = applyReview(fields, { correct: true }, T0 + 5000)
    expect(result.srsBox).toBe(2)
  })

  it("demotes on incorrect", () => {
    const fields: SrsFields = { srsBox: 3, nextReviewAt: T0, reviewCount: 3, lastReviewedAt: T0 - 1000 }
    const result = applyReview(fields, { correct: false }, T0 + 5000)
    expect(result.srsBox).toBe(1)
  })
})
