...
function hasAdvancedRules(siteRule?: SiteConfig): boolean {
  return !!siteRule?.selectors?.length
    ...
    || !!siteRule?.includePathPatterns?.length
    || !!siteRule?.excludePathPatterns?.length
...
export default function SiteSettingsSection({
  ...
  const [includePathPatternsValue, setIncludePathPatternsValue] = useState(() => toMultilineValue(rawSiteRule?.includePathPatterns))
  const [excludePathPatternsValue, setExcludePathPatternsValue] = useState(() => toMultilineValue(rawSiteRule?.excludePathPatterns))
  ...
  useEffect(() => {
    ...
    setIncludePathPatternsValue(toMultilineValue(rawSiteRule?.includePathPatterns))
    setExcludePathPatternsValue(toMultilineValue(rawSiteRule?.excludePathPatterns))
    setSelectorsError(null)
    setExcludeSelectorsError(null)
    setCustomCssValue(rawSiteRule?.customCss ?? "")
  }, [
    activeSiteKey,
    rawSiteRule?.selectors,
    rawSiteRule?.excludeSelectors,
    rawSiteRule?.includePathPatterns,
    rawSiteRule?.excludePathPatterns,
    rawSiteRule?.customCss,
  ])
  ...
  const scopeLabel = rawSiteRule?.contentScope ?? INHERIT_VALUE
  const summaryChips = [
    `scope: ${scopeLabel === INHERIT_VALUE ? "inherit" : scopeLabel}`,
    `min ¶ ${rawSiteRule?.paragraphMinLength ?? "inherit"}`,
    `include ${rawSiteRule?.selectors?.length ?? 0}`,
    `exclude ${rawSiteRule?.excludeSelectors?.length ?? 0}`,
    `paths ${(rawSiteRule?.includePathPatterns?.length ?? 0) + (rawSiteRule?.excludePathPatterns?.length ?? 0)}`,
  ]
  ...
  return (
    <details open style={{ marginBottom: 12 }}>
      ...
      <div className="astra-site-sheet" style={{ marginTop: 8 }}>
        <div className="astra-site-sheet__hero">
          <span className="astra-site-sheet__icon" aria-hidden="true">🌐</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="astra-site-sheet__title">{activeSiteKey}</div>
            <div className="astra-site-sheet__meta">{advancedConfigured ? "Custom rule" : "Global defaults"}</div>
          </div>
          {(rawSiteRule || advancedConfigured) && <span className="astra-chip-warm">Custom</span>}
        </div>
        ...
        <div className="astra-site-sheet__card">
          ...
          <label htmlFor="site-auto-translate-checkbox" style={{ ...checkboxRowStyle, marginTop: 8 }}>
            <input
        ...
        <div className="astra-site-sheet__card">
          <div className="astra-sheet-row">
            <div className="astra-sheet-row__copy">
              <div className="astra-sheet-row__title">Scope summary</div>
              <div className="astra-sheet-row__sub">Quick view of filters and path matching.</div>
            </div>
          </div>
          <div className="astra-site-sheet__chips" style={{ marginTop: 8 }}>
            {summaryChips.map((chip) => (
              <span key={chip} className="astra-site-sheet__chip">{chip}</span>
            ))}
          </div>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                <button
                  ...
                  style={{ flex: "none", padding: "3px 8px", fontSize: 11, opacity: detectingArticle ? 0.6 : 1 }}
                >
              ...
              {selectorsError && <div data-testid="site-selectors-error" style={warningStyle}>{selectorsError}</div>}
              ...
              />
              <div style={{ fontSize: 11, color: "var(--astra-text-muted)", marginTop: 4 }}>{t("hint_siteSelectorsPerLine")}</div>
              {excludeSelectorsError && <div data-testid="site-exclude-selectors-error" style={warningStyle}>{excludeSelectorsError}</div>}

              <label htmlFor="site-include-path-patterns-input" style={labelStyle}>Include path patterns</label>
              <textarea
                id="site-include-path-patterns-input"
                data-testid="site-include-path-patterns-input"
                value={includePathPatternsValue}
                onChange={(e) => {
                  const nextValue = e.target.value
                  setIncludePathPatternsValue(nextValue)
                  const includePathPatterns = fromMultilineValue(nextValue)
                  onSiteRuleChange((siteRule) => {
                    const nextSiteRule: SiteConfig = { ...siteRule }
                    if (includePathPatterns) {
                      nextSiteRule.includePathPatterns = includePathPatterns
                    } else {
                      delete nextSiteRule.includePathPatterns
                    }
                    return nextSiteRule
                  })
                }}
              ...
              <label htmlFor="site-exclude-path-patterns-input" style={labelStyle}>Exclude path patterns</label>
              <textarea
                id="site-exclude-path-patterns-input"
                data-testid="site-exclude-path-patterns-input"
                value={excludePathPatternsValue}
                onChange={(e) => {
                  const nextValue = e.target.value
                  setExcludePathPatternsValue(nextValue)
                  const excludePathPatterns = fromMultilineValue(nextValue)
                  onSiteRuleChange((siteRule) => {
                    const nextSiteRule: SiteConfig = { ...siteRule }
                    if (excludePathPatterns) {
                      nextSiteRule.excludePathPatterns = excludePathPatterns
                    } else {
                      delete nextSiteRule.excludePathPatterns
                    }
                    return nextSiteRule
                  })
                }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input
                  ...
                  disabled={!(rawSiteRule?.enabled ?? true)}
                />
                <span style={{ fontSize: 12, color: "var(--astra-text-muted)", minWidth: 24, textAlign: "right" }}>{paragraphSliderValue}</span>
              </div>
        ...
        <div className="astra-site-sheet__card">
          ...
          {ruleStatus && <div data-testid="site-rule-status" style={{ fontSize: 11, color: "var(--astra-success)", marginTop: 6 }}>{ruleStatus}</div>}
        </div>
      </div>
    </details>
  )
