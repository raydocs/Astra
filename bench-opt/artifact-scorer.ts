...
export function collectCurrentArtifactEvidence(
  ...
  // --- 4. Count artifacts in bench-live-results ---
  try {
    const resultsDir = resolve(projectRoot(), "bench-live-results")
  ...
  // --- 6. Live results from bench-live-results ---
  try {
    const resultsDir = resolve(projectRoot(), "bench-live-results")
...
function projectRoot(): string {
  ...
  // In practice we resolve relative to cwd which is the project root.
