...
async function main() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    ...
    console.log("  Loads the latest session/checkpoint/handoff bundle from bench-opt-results/store/index.json")
    console.log("  and forwards it into bench-opt runner with --orchestrate --session enabled by default.")
    return
  }

  const outputDir = path.resolve(process.cwd(), "bench-opt-results")
