...
function DesignCanvas({ children, minScale, maxScale, style, title, subtitle }) {
    ...
    <DCCtx.Provider value={api}>
      <DCViewport minScale={minScale} maxScale={maxScale} style={style}>
        {ready && (
          <>
            {(title || subtitle) && (
              <div style={{ padding: "0 60px 36px", width: "max-content", maxWidth: 920 }}>
                {title && (
                  <div style={{ fontSize: 26, fontWeight: 650, color: DC.title, letterSpacing: -0.5, lineHeight: 1.15 }}>
                    {title}
                  </div>
                )}
                {subtitle && (
                  <div style={{ fontSize: 15, color: DC.subtitle, marginTop: 10, lineHeight: 1.55, maxWidth: 720 }}>
                    {subtitle}
                  </div>
                )}
              </div>
