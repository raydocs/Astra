...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let runs = 3
  let sprints = 5
  let outputDir = path.resolve(process.env.ASTRA_BENCH_OPT_ARTIFACT_ROOT ?? path.join(process.cwd(), "data/bench-opt-results"), "proof-suite")
...
function printHelp() {
  ...
  console.log("")
  console.log("Usage: npx tsx script/bench-opt/proof-suite-entry.ts [options]")
  console.log("")
  console.log("Options:")
  console.log("  --runs, -r <n>             Runs per prompt (default: 3)")
  console.log("  --sprints, -s <n>          Sprints per run (default: 5)")
  console.log("  --output-dir, -o <path>    Output directory (default: data/bench-opt-results/proof-suite)")
