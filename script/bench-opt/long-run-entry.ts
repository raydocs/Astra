...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let prompt = ""
  let sprints = 5
  let outputDir = path.resolve(process.env.ASTRA_BENCH_OPT_ARTIFACT_ROOT ?? path.join(process.cwd(), "data/bench-opt-results"), "long-run")
...
function printHelp() {
  ...
  console.log("  --output-dir, -o <path>    Output directory (default: data/bench-opt-results/long-run)")
