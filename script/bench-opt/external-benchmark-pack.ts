...
export function createOfficialBenchmarkPack(): Readonly<BenchmarkPackConfig> {
  const pack: BenchmarkPackConfig = {
    ...
    runbook: {
      ...
      executeCommand:
        "npx tsx script/bench-opt/proof-suite-entry.ts --runs 3 --sprints 5 --output-dir data/bench-opt-results/proof-suite",
      verifyCommand:
      ...
      artifactDir: "data/bench-opt-results/proof-suite",
      reportPath: "data/bench-opt-results/proof-suite/latest.proof-suite.md",
    },

    artifactLayout: {
      root: "data/bench-opt-results/proof-suite",
      perRun: [
...
export function createDraftCapabilityBenchmarkPackV2(): Readonly<BenchmarkPackConfig> {
  ...
  const pack: BenchmarkPackConfig = {
    ...
    runbook: {
      ...base.runbook,
      verifyCommand:
        "npx tsx script/bench-opt/external-benchmark-pack-entry.ts --pack v2 --validate data/bench-opt-results/proof-suite/latest.proof-suite.json --status data/bench-opt-results/latest.status.json",
      reportPath: "data/bench-opt-results/latest.status.md",
    },
    requiredCapabilities: [
      {
        id: "privacy-mode",
