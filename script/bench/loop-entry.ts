...
async function readLatestHandoff() {
  const handoffPath = path.join(path.resolve(process.env.ASTRA_BENCH_ARTIFACT_ROOT ?? path.join(process.cwd(), "data/bench-results")), "latest.handoff.json")
...
export async function runLoop(argv: string[] = process.argv.slice(2)) {
  ...
  const optimizer = await loadResolvedOptimizerConfig(
    optimizerConfigPath ?? (useBenchOpt ? path.join(path.resolve(process.env.ASTRA_BENCH_OPT_ARTIFACT_ROOT ?? path.join(process.cwd(), "data/bench-opt-results")), "latest.resolved.json") : null),
  )
