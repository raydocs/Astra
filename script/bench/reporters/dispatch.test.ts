...
const readyExecutor: ExecutorAttempt = {
  ...
  sourceArtifacts: {
    latestPatchPass: "data/bench-results/latest.patch-pass.json",
...
describe("dispatch reporter", () => {
  ...
  it("renders blocked dispatches without a response", () => {
    const artifact = buildDispatchArtifact({
      ...
      sourceArtifacts: {
        latestExecutor: "data/bench-results/latest.executor.json",
        latestPatchPass: "data/bench-results/latest.patch-pass.json",
        latestPatchContext: "data/bench-results/latest.patch-context.md",
      },
      prompt: null,
    })
